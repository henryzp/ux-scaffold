...
KISSY.add("components/mock/index", function(S) {
  ...
  KISSY.io = function(options) {
    ...
    for (var i = 0; i < _mocked.length; i++) {
      ...
      if (mock.request.test(options.url)) {
        console.log('mock data & ajax for: ' + options.url);
        var data = Mock.mockJSON.generateFromTemplate(mock.template);
        console.log(data);
  ...
  function type(obj) {
    return S.isArray(obj) ? 'array' : (obj === null) ? 'null' : typeof obj;
  }

  Mock.mockJSON.generateFromTemplate = function(template, name) {
    var length = 0,
      i;
    ...
    switch (type(template)) {
      case 'array':
        generated = [];
        for (i = 0; i < length; i++) {
          generated[i] = Mock.mockJSON.generateFromTemplate(template[0]);
        }
        break;
      ...
      case 'object':
        ...
        for (var p in template) {
          generated[p.replace(/\|(\d+-\d+|\+\d+)/, '')] = Mock.mockJSON.generateFromTemplate(template[p], p);
  ...
  function getRandomData(key) {
    ...
    if (!(key in Mock.mockJSON.data)) {
      console.log(key);
      console.log(params);
      return key;
    }

    var a = Mock.mockJSON.data[key];
