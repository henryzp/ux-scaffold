...
var RootEvents={};
var VframeTag=Magix.config().tagName.toUpperCase();
...
var Body=MIX({
    ...
    processEvent:function(e){
        ...
        if(!eventReg.test(target.getAttribute(MxIgnore))){
            ...
            if(info){//有事件
                //找处理事件的vframe
                var handler=current.getAttribute(MxOwner);
                ...
                if(handler){//有处理的vframe,派发事件，让对应的vframe进行处理
                    ...
                    throw new Error('miss '+MxOwner+':'+info);
                }
            }else{
                var node;
                var ignore;
...
var InSubVframs=function(v,a,i,r){
...
Mix(Vframe,{
    ...
    createNode:function(id){
        ...
        vfNode.id=id;
        return vfNode;
        //anchor.insertBefore(vfNode,anchor.firstChild);
    }
});
/*
    修正IE下标签问题
...
Mix(Vframe.prototype,{
            ...
            Magix.libRequire(vn,function(View){
                if(sign==me.sign){//有可能在view载入后，vframe已经卸载了
                    ...
                    view.on('interact',function(e){//view准备好后触发
                        me.fire('viewLoad',{view:view},me.viewUsable=1);
                        /*
    ...
    mountZoneVframes:function(zoneId,viewInitParams,_byHand){
        ...
        var subs=[];
        if(count){
            for(var i=0,vframe,mxView,vf;i<count;i++){
                vframe=vframes[i];
                //
                if(!InSubVframs(vframe,subs)){
                    mxView=vframe.getAttribute(DataView);
                    vf=Vframe.create(vframe,{
                        owner:me.owner,
                        pId:me.id
                    });
                    if(!Has(me.cS,vf.id)){
                    ...
                    me.cS[vf.id]=_byHand;
                    vf.mountView(mxView,viewInitParams);
                    owner.add(vf);
                }
 ...
 * @property {String} template 当前view对应的模板字符串(当hasTmpl为true时)，该属性在created事件触发后才存在
...
Mix(VProto,{
    ...
    load:function(){
        ...
        var ready=function(){
            ...
            if(noTemplateAndNoRendered){//监视有没有在调用render方法内使用setViewHTML更新view，对于没有模板的view，是不需要调用的，此时我们需要添加不冒泡的事件处理，如果调用了，则在setViewHTML中处理，首次就不再处理了，只有冒泡的事件才适合在首次处理
                me.rendered=true;
                me.fire('created',null,1);//created事件只触发一次
            }
        };
    ...
    observeLocation:function(args){
        ...
        if(Magix.isObject(args)){
            ...
            keys=keys.concat(args.keys);
        }else{
            keys=keys.concat(Magix.isString(args)?args.split(COMMA):args);
        }
        loc.keys=keys;
    },
    /**
    ...
    parentView:function(){
        ...
        var pVframe=vom.getVframe(owner.pId),r=null;
...
/*    signature:function(){
     ...
     * @name View#rendered
     * @event
     * @param {Object} e view每次调用setViewHTML完成后触发，当hasTmpl属性为false时，并不会触发该事 件，但会触发created首次完成创建界面的事件
     */
    ...
    getRoot:function(){
        ...
        if(!vf){
            ...
            if(!rootVframeNode){//当发现不存在的节点时，由Vframe对象负责创建
                vf=D.body;
                vf.insertBefore(Vframe.createNode(rootVframeId),vf.firstChild);
            }
            vf=Vframe.create(rootVframeId,{owner:VOM});
            VOM.add(vf);
        }
        return vf;
    },
        ...
        fetchModels:function(models,done,flag){
            ...
            for(var i=0,model;i<models.length;i++){
                ...
                if(model){
                    ...
                    var cacheKey=modelInfo._cacheKey;
